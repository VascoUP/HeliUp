...
public enum ItemEnum {
    UP_VELOCITY {
        ...
        GameObject getItem(Random random, float max, float min, Object... params) throws OccupiedPositionError {
            ...
            return new IncreaseVelocity(new Vector2(helicopter.getPosition().x + ScreenInfo.width - 200f, random.nextFloat() * max + min));
        }
    }, UP_CAPACITY {
        @Override
        GameObject getItem(Random random, float max, float min, Object... params) throws OccupiedPositionError {
            Helicopter helicopter = Game.getGame().getHelicopter();
            return new IncreaseCapacity(new Vector2(helicopter.getPosition().x + ScreenInfo.width - 200f, random.nextFloat() * max + min));
        }
    };

    abstract GameObject getItem(Random random, float max, float min, Object... params) throws OccupiedPositionError;

    public static GameObject randomItem(Random random, float max, float min, Object... params) throws OccupiedPositionError {
